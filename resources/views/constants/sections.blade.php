...
<div class="container-fluid px-2">
    <div class="programs-table-card" style="width:100%;background:#faf9f6;border-radius:16px;box-shadow:0 4px 24px rgba(30,41,59,0.07);padding:1.4rem 1.2rem;border-top:5px solid #d4af37;">
        <h2 style="font-weight:900;color:#174032;font-size:1.4rem;letter-spacing:0.5px;font-family:'Cairo',sans-serif;text-align:center;margin-bottom:1.4rem;">الثوابت - الأقسام</h2>
        ...
        <form method="POST" action="{{ route('constants.sections.store') }}" class="mb-3">
            @csrf
            <div class="row g-3">
                <div class="col-md-4">
                    <label class="form-label" style="color:#174032;font-weight:700;font-family:'Cairo',sans-serif;">اسم القسم</label>
                    <input type="text" name="name" value="{{ old('name') }}" class="form-control" placeholder="مثال: الشؤون التعليمية" style="border-radius:8px;border:1.5px solid #d4af37;font-family:'Cairo',sans-serif;">
                </div>
                <div class="col-md-7">
                    <label class="form-label" style="color:#174032;font-weight:700;font-family:'Cairo',sans-serif;">الوصف</label>
                    <input type="text" name="description" value="{{ old('description') }}" class="form-control" placeholder="وصف مختصر عن القسم" style="border-radius:8px;border:1.5px solid #d4af37;font-family:'Cairo',sans-serif;">
                </div>
            ...
            <table class="announcements-table" style="width:100%;border-collapse:separate;border-spacing:0 0.5rem;font-size:1.07rem;font-family:'Cairo',sans-serif;">
                ...
                <tbody>
                    ...
                    <tr class="announcements-row" style="background:#fff;transition:background 0.18s;border-radius:8px;">
                        <td style="padding:1rem;text-align:center;font-weight:500;">{{ $s->id }}</td>
                        <td style="padding:1rem;text-align:center;font-weight:600;">{{ $s->name }}</td>
                        <td style="padding:1rem;text-align:center;">{{ $s->description }}</td>
                        <td class="text-center d-flex gap-2 justify-content-center">
                            <a href="{{ route('constants.sections.edit', $s) }}" class="btn btn-sm btn-warning icon-btn" title="تعديل"><i class="fas fa-edit"></i></a>
                            <form action="{{ route('constants.sections.destroy', $s) }}" method="POST" class="d-inline" onsubmit="return confirm('هل أنت متأكد من الحذف؟');">
                                @csrf
                                @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center">لا توجد أقسام مسجلة بعد</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<style>
    /* Page-local: widen working area on Sections */
