...
<x-breadcrumb :items="[
    ['title' => 'الثوابت'],
    ['title' => 'المعلمين', 'url' => route('constants.teachers')],
    ['title' => 'تعديل المعلم']
]" />
...
<div class="container-fluid px-2" style="min-height:80vh; display:flex; align-items:center; justify-content:center;">
    <div class="programs-table-card" style="width:100%;background:#faf9f6;border-radius:16px;box-shadow:0 4px 24px rgba(30,41,59,0.07);padding:1.4rem 1.2rem;border-top:5px solid #d4af37;">
        <h2 style="font-weight:900;color:#174032;font-size:1.4rem;letter-spacing:0.5px;font-family:'Cairo',sans-serif;text-align:center;margin-bottom:1.4rem;">تعديل المعلم</h2>
        ...
        <form method="POST" action="{{ route('constants.teachers.update', $teacher) }}">
            @csrf
            @method('PUT')
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select name="masjid_id" class="form-select" style="border-radius:8px;border:1.5px solid #d4af37;font-family:'Cairo',sans-serif;">
                        @foreach($masjids as $m)
                            <option value="{{ $m->id }}" {{ old('masjid_id', $teacher->masjid_id) == $m->id ? 'selected' : '' }}>{{ $m->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
                    <label class="form-label" style="color:#174032;font-weight:700;font-family:'Cairo',sans-serif;">اسم المعلم</label>
                    <input type="text" name="name" value="{{ old('name', $teacher->name) }}" class="form-control" placeholder="اسم المعلم" style="border-radius:8px;border:1.5px solid #d4af37;font-family:'Cairo',sans-serif;">
                </div>
            </div>
            <div class="d-flex align-items-center mt-4">
                ...
                <a href="{{ route('constants.teachers') }}" class="btn ms-auto" style="border-radius:8px;background:#e2e8f0;color:#174032;font-weight:700;font-family:'Cairo',sans-serif;">عودة</a>
            </div>
        </form>
    </div>
</div>
