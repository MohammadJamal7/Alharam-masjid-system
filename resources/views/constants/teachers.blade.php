...
<div class="container-fluid px-2">
    <div class="programs-table-card" style="width:100%;background:#faf9f6;border-radius:16px;box-shadow:0 4px 24px rgba(30,41,59,0.07);padding:1.4rem 1.2rem;border-top:5px solid #d4af37;">
        <h2 style="font-weight:900;color:#174032;font-size:1.4rem;letter-spacing:0.5px;font-family:'Cairo',sans-serif;text-align:center;margin-bottom:1.4rem;">الثوابت - المعلمين</h2>
        ...
        <form method="POST" action="{{ route('constants.teachers.store') }}" class="mb-3">
            @csrf
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" class="form-control" placeholder="اسم المعلم" style="border-radius:8px;border:1.5px solid #d4af37;font-family:'Cairo',sans-serif;">
                </div>
            ...
            <table class="announcements-table" style="width:100%;border-collapse:separate;border-spacing:0 0.5rem;font-size:1.07rem;font-family:'Cairo',sans-serif;">
                ...
                <tbody>
                    ...
                    <tr class="announcements-row" style="background:#fff;transition:background 0.18s;border-radius:8px;">
                        <td style="padding:1rem;text-align:center;font-weight:500;">{{ $t->id }}</td>
                        <td style="padding:1rem;text-align:center;font-weight:600;">{{ $t->masjid?->name }}</td>
                        <td style="padding:1rem;text-align:center;font-weight:600;">{{ $t->name }}</td>
                        
                        <td class="text-center d-flex gap-2 justify-content-center">
                            <a href="{{ route('constants.teachers.edit', $t) }}" class="btn btn-sm btn-warning icon-btn" title="تعديل"><i class="fas fa-edit"></i></a>
                            <form action="{{ route('constants.teachers.destroy', $t) }}" method="POST" class="d-inline" onsubmit="return confirm('هل أنت متأكد من الحذف؟');">
                                @csrf
                                @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center">لا يوجد معلمون مسجلون بعد</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
